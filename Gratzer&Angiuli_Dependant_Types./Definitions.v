...
Fixpoint is_slType (t : slType) : Prop :=
  match t with
  | Base => True
  | Function A B => is_slType A /\ is_slType B
  | Prod A B => is_slType A /\ is_slType B
  end.

Lemma is_slType_base : is_slType Base.
Proof.
   simpl. auto.
Qed.

Lemma is_slType_function (A B : slType) :
  is_slType A /\ is_slType B -> is_slType (A --> B).
Proof.
   simpl. auto.
Qed.

Lemma is_slType_pair (A B : slType) :
  is_slType A /\ is_slType B -> is_slType (A ** B).
Proof.
   simpl. auto.
Qed.
...
Example ex1 : is_slType ((Base ** Base) --> Base).
Proof.
  simpl. auto.
Qed.


Fixpoint is_slContext (ctx : slContext) : Prop :=
  match ctx with
  | Empty => True
  | Extend c _ t => is_slContext c /\ is_slType t
  end.
